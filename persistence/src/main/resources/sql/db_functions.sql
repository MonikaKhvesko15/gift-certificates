...
begin
    ...
    case
        ...
        when ((tag_name IS NOT NULL) AND (context IS NOT NULL)) then
            return query
                ...
                WHERE
                    (gift_certificates.isdeleted = 0) AND
                    ((gift_certificates.name ILIKE concat('%', context, '%')) OR
                     (gift_certificates.description ILIKE concat('%', context, '%'))) AND
                    (tags.name ILIKE concat('%', tag_name, '%'));

        when((tag_name IS NULL) AND (context IS NOT NULL)) then
            return query
                select distinct gift_certificates.id,
                                gift_certificates.name,
                                gift_certificates.description,
        ...
        when((tag_name IS NOT NULL) AND (context IS NULL))then
            return query
        ...
        when((tag_name IS NULL) AND (context IS NULL))then
            return query
                select distinct gift_certificates.id,
                                gift_certificates.name,
                                gift_certificates.description,
